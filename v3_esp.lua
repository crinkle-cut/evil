...
local esp = {
    ...
    -- instances
    --\ @teammates (same team - green for TTT)
    team_chams = { false, Color3.new(0, 1, 0), Color3.new(0, 1, 0), .25, .75, true },
    --
    team_boxes = { false, Color3.new(0, 1, 0), Color3.new(0, 1, 0), 0.95 },
    team_healthbar = { false, Color3.new(0, 1, 0), Color3.new(1, 1, 0) },
    team_kevlarbar = { false, Color3.new(0, 0, 1), Color3.new(0, 1, 1) },
    ...
    --\ @enemies (traitors for others, non-traitors for traitors - red for TTT)
    enemy_chams = { false, Color3.new(1, 0, 0), Color3.new(1, 0, 0), .25, .75, true },
    --
    enemy_boxes = { false, Color3.new(1, 0, 0), Color3.new(1, 0, 0), 0.95 },
    enemy_healthbar = { false, Color3.new(1, 0, 0), Color3.new(1, 1, 0) },
    enemy_kevlarbar = { false, Color3.new(1, 0.5, 0), Color3.new(1, 1, 0) },
    ...
    --\ @priority (detectives - blue for TTT)
    priority_chams = { false, Color3.new(0, 0, 1), Color3.new(0, 0, 1), .25, .75, true },
    --
    priority_boxes = { false, Color3.new(0, 0, 1), Color3.new(0, 0, 1), 0.95 },
    priority_healthbar = { false, Color3.new(0, 0, 1), Color3.new(0, 1, 1) },
    priority_kevlarbar = { false, Color3.new(0, 0, 1), Color3.new(0, 1, 1) },
    priority_arrow = { false, Color3.new(0, 0, 1), 0.5 },
    --
    priority_names = { false, Color3.new(0, 0, 1)},
    priority_weapon = { false, Color3.new(0, 0, 1)},
...
-- TTT game integration
local ReplicatedStorage = game:GetService("ReplicatedStorage")

-- TTT modules
local teams = require(ReplicatedStorage.SharedModules.Teams)

function esp.getcharacter(plr)
    -- TTT uses ReplicationFocus instead of Character
    local replication_focus = plr.ReplicationFocus
    if replication_focus then
        -- Character model is in the Visuals subfolder
        return replication_focus:FindFirstChild("Visuals")
    end
    return nil
end
...
function esp.checkteam(plr, bool)
    ...
    -- TTT team logic
    local localTeam = teams[localPlayer]
    local playerTeam = teams[plr]

    if not localTeam or not playerTeam then return true end -- show if team unknown

    -- Team-based enemy detection for TTT
    if localTeam == "Traitor" then
        -- Traitors see non-traitors as enemies
        return playerTeam ~= "Traitor"
    elseif localTeam == "Detective" then
        -- Detectives see traitors as enemies
        return playerTeam == "Traitor"
    else -- Innocent
        -- Innocents see traitors as enemies (if they know)
        return playerTeam == "Traitor"
    end
end
...
function esp:checkvisible(instance, origin, params)
    ...
    -- TTT uses ReplicationFocus, so filter it instead of Character
    local hit = self:raycast(camera.CFrame.p, (origin.Position - camera.CFrame.p).unit * 500, { unpack(params), camera, localPlayer.ReplicationFocus })
    return (hit and hit.Instance:IsDescendantOf(instance)) and true or false
end

-- TTT weapon detection function
function esp.getweapon(plr)
    local character = esp.getcharacter(plr)
    if not character then return nil end

    -- TTT weapon detection using Tool system
    ...
    for plr, drawing in next, esp.players do
        ...
        if esp.enabled and esp.checkalive(player) then
            ...
            if drawing.weapon.Visible then
                -- TTT weapon detection
                local weapon_name = esp.getweapon(plr_instance)
                drawing.weapon.Text = weapon_name or "unarmed"
